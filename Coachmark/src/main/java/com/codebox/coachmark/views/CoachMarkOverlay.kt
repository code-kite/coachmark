...
import com.codebox.coachmark.constants.Shape
import com.codebox.coachmark.constants.ShapeType
import com.codebox.coachmark.constants.Utils
import com.codebox.coachmark.utils.Util
import kotlin.math.roundToInt

/**
 * This class is used to design and build coach mark overlay
 */
class CoachMarkOverlay : FrameLayout {
    ...
    private fun drawOverlayTint() {
        ...
        if (width == 0 || height == 0) { // handling exception height > 0
            finalWidth = Util.getScreenWidth(context)
            finalHeight = Util.getScreenHeight(context)
        }
        mBaseBitmap = Bitmap.createBitmap(finalWidth, finalHeight, Bitmap.Config.ARGB_8888)
        ...
        mLayer?.drawRect(Rect(0, 0, finalWidth, finalHeight), mOverlayTintPaint)
    }
        ...
        mBuilder.getInfoViewBuilder()?.apply {
            ...
            if (mTooltip != null) {
                        ...
                        when (mBuilder.getToolTipBuilder()?.getToolTipShape()) {
                            Shape.TRIANGLE -> {
                                lp.topMargin = targetSize.bottom + Utils.dpToPx(context, 8).roundToInt() +
                                        mBuilder.getOverlayTransparentPadding().bottom + getMargin().top
                        ...
                        when (mBuilder.getToolTipBuilder()?.getToolTipShape()) {
                            Shape.TRIANGLE -> {
                                ...
                                lp.leftMargin =
                                        targetSize.centerX() - mBuilder.getToolTipBuilder()!!.getToolTipWidth() / 2
                                infoTextLayoutParams.topMargin =
                                        targetSize.top - mBuilder.getInfoViewBuilder()!!.getInfoViewHeight() - Utils.dpToPx(context, 8).roundToInt() - getMargin().bottom
                                lp.topMargin = infoTextLayoutParams.topMargin + mBuilder.getInfoViewBuilder()!!.getInfoViewHeight()
                            }
                            Shape.CIRCLE -> {
                                lp.width = mBuilder.getToolTipBuilder()!!.getToolTipWidth()
                                lp.height = mBuilder.getToolTipBuilder()!!.getToolTipHeight() / 2
                ...
                */
                ...
                when (getInfoViewGravity()) { // This is used to set info view gravity
                    Gravity.BOTTOM -> {
                        infoTextLayoutParams.topMargin =
                    ...
                    Gravity.TOP -> {
                        infoTextLayoutParams.topMargin = targetSize.top - Utils.dpToPx(
                                context,
                                8
                        ).roundToInt() - mBuilder.getInfoViewBuilder()!!.getInfoViewHeight() - mBuilder.getOverlayTransparentPadding().top - getMargin().bottom
                    }
                    else -> {}
                }
                /**
